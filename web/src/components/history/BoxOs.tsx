  ...
  return (
    <div className="flex flex-col gap-4">
      {ordens.map((os) => {
        ...
        return (
          <div key={os.numOs} className="flex items-center justify-between bg-[#0F0F0F] border border-[#222222] rounded-[24px] p-5 md:p-6 hover:border-[#333333] transition-colors cursor-pointer group">
            <div className="flex items-center gap-5">
              ...
              <div className="flex flex-col">
                <span className="text-zinc-500 text-xs md:text-sm font-medium mb-0.5">#{os.numOs}</span>
                <span className="text-white font-bold text-base md:text-lg leading-none mb-1">{os.cliente.nmCompleto}</span>
                <span className="text-zinc-400 text-xs md:text-sm">{os.aparelho.modelo} • {new Date(os.dtEntrada).toLocaleDateString("pt-BR")}</span>
              </div>
            </div>
